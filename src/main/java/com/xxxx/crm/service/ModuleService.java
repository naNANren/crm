...
public class ModuleService extends BaseService<Module,Integer> {
    ...
    public List<TreeModel> queryAllModules(Integer roleId){
      ...
      List<Integer> permissionIds=permissionMapper.queryRoleHasModuleIdsByRoleId(roleId);
        ...
        if (permissionIds != null && permissionIds.size()>0){
            //删除所有的资源列表,判断用户拥有的资源ID中是否有匹配的,如果有,则设置checked属性为true
    ...
    public void addModule(Module module){
       ...
       if (grade == 1) {
           ...
           //地址url     二级菜单  (grade=1)  同一层级下不可重复
           AssertUtil.isTrue(null != moduleMapper.queryModuleByGradAndUrl(grade, module.getUrl()), "URL不可重复");
       }
    ...
    public void updateModule(Module module){
        ...
        Integer grade=module.getGrade();
        AssertUtil.isTrue(null==grade || !(grade == 0 || grade ==1 || grade ==2),"菜单层级不合法!");
        //模块名称 moduleName  非空   模块名称 moduleName 同一层级下模块名唯一  (不包含当前修改记录本身)
        AssertUtil.isTrue(StringUtils.isBlank(module.getModuleName()),"模块名称不能为空!");
        //通过层级与模块名称查询资源对象
        ...
        if (grade ==1){
            ...
            //通过层级和模块名称查询资源对象
            temp=moduleMapper.queryModuleByGradAndUrl(grade,module.getUrl());
            //判断是否存在
            if (temp != null){
     ...
     *    1.判断删除的记录是否存在
     *    2.如果当前资源存在子记录,则不可删除
     *    3.删除资源时,将对应的权限表的记录也删除(判断去权限表中是否存在关联数据,如果存在,则删除)
    ...
    public void deleteModule(Integer id) {
           ...
           //如果当前资源存在子记录,则不可删除 (将id当做父id查询记录)
