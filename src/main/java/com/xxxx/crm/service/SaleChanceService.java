...
public class SaleChanceService extends BaseService<SaleChance,Integer> {
     ...
     *   assignMan指派人
     *       如果未设置指派人(默认)
     *          satte分配状态  (0=未分配, 1=已分配)
     *               未分配
     *         assignTime 指派时间
     *               设置为null
     *            devResult开发状态(0=未开发 ,1=开发中,3=开发失败)
     *         如果设置了指派人
     *            state分配状态 ( (0=未分配, 1=已分配))
     *                1=已分配
     *                assignTime指派时间
     *                    系统当前时间
     *            isValid是否有效  (0=失效,1=有效)
     *              设置为有效  1=有效
     *            createDate创建时间
     *                默认是系统当前时间
     *            updateDate
    ...
    //判断是否设置了指派人
        if (StringUtils.isBlank(saleChance.getAssignMan())){
            //如果为空,则表示未设置指派人
            //sate分配状态 (0=未分配 1=已分配) 0=未分配
            saleChance.setState(StateStatus.UNSTATE.getType());
            //assignTime 指派时间 设置为null
            saleChance.setAssignTime(null);
            // devResult开发状态(0=未开发 ,1=开发中,3=开发失败)  0=未开发(默认)
          saleChance.setDevResult(DevResult.UNDEV.getStatus());
        }else {
            //如果不为空,则表示设置了指派人
            //   state分配状态 ( (0=未分配, 1=已分配))1=已分配
          saleChance.setState(StateStatus.STATED.getType());
            //    assignTime指派时间系统当前时间
            saleChance.setAssignTime(new Date());
         //  devResult开发状态(0=未开发 ,1=开发中,3=开发失败)   1=开发中
